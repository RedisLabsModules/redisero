...
def sh(cmd, join=False, lines=False, fail=True):
    ...
    if proc.returncode != 0 and fail is True:
        raise Exception(err.decode("utf-8"), out=out, retval=proc.returncode)
    return out
...
class Platform:
    ...
    def __init__(self, strict=False, brand=False):
        self.os = (
            self.dist
        ) = self.os_ver = self.os_full_ver = self.osnick = self.arch = "?"
        self.strict = strict
        self.brand_mode = brand
    ...
    def _identify_linux(self):
        try:
            ...
            if self.strict:
                raise Exception("Cannot determine distribution")
    ...
    def _identify_linux_dist(self, os_release):
        ...
        if distname == "fedora" or distname == "debian":
            ...
            or distname.startswith("rocky")
            or distname.startswith("almalinux")
        ):
            distname = "centos"
    ...
    def _identify_macos(self):
        ...
        self.osnick = DARWIN_VERSIONS_NICKS.get(
            self.darwin_ver.split(".")[0], self.os + str(self.os_ver)
        )
    ...
    def os_version(self):
        return tuple(
            map(lambda x: int(x) if is_numeric(x) else x, self.os_full_ver.split("."))
        )
